...
try:
    ...
    if sel_h:
        with c2:
            ...
            sel_u = st.selectbox("Unité UG", u_opts, index=None, placeholder="N° UG...")
        ...
        if sel_u:
            ...
            with col2:
                st.markdown(f'<div class="st-card"><div class="t-lbl">Immeuble</div><div class="t-val">{int(s_i):,} m²</div><div class="t-desc">{len(du[du["GROUPE (HP2)"] == sel_h])} Unités</div></div>', unsafe_allow_html=True)
            ...
            st.markdown(f'<div class="st-card"><span class="tag {t_sty}">{t_lbl}</span><div class="t-lbl">Chauffage</div><div class="t-val" style="font-size:1.7rem;">{fuel}</div><div style="color:#6B7280; font-size:0.85rem; margin-top:5px;">{eq}</div></div>', unsafe_allow_html=True)

except Exception as e:
    st.info("Sélectionnez un groupe pour charger les données.")
